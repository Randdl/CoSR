...
from .hat_arch_hf_ECAB_mw import RHAG_wave as RHAG
...
class Wave_Down(nn.Module):
    ...
    def __init__(self, channels,  dims=2, out_channels=None):
        ...
        self.out_channels = out_channels or channels
        self.dims = dims
        self.DWT = DWTForward(J=2, wave='rbio1.1',mode='periodization').cuda()
    ...
    def _transformer(self, DMT1_yh):
        ...
        for i in range(3):
            list_tensor.append(nn.functional.interpolate(DMT1_yh[1][:,:,i,:,:], scale_factor=2, mode='bilinear'))
...
class XIXISR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
        ...
        self.conv_hf = nn.Conv2d(num_in_ch*6, embed_dim, 3, 1, 1)
        self.conv_coor = nn.Conv2d(1024, embed_dim, 3, 1, 1)
        self.conv_merge = nn.Conv2d(embed_dim*3, embed_dim, 1, 1, 0)
    ...
    def forward(self, x, scale, resolution):
        ...
        x  = torch.cat([coords_x, x,wave_hf], 1)
        # print(x.shape)
        x = self.conv_merge(x)
        # print(x.shape)
