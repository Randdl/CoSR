...
class HAB_wave(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_block = CAB(num_feat=dim*4, compress_ratio=compress_ratio, squeeze_factor=squeeze_factor)
    ...
    def forward(self, x, x_size, rpi_sa, attn_mask):
        ...
        x = x.permute(0, 3, 1, 2)
        # Conv_X
        cab_x = self.wave_down(x).contiguous()
        x = cab_x.view(b, int(h/2),int(w/2), int(c*4))
        cab_x = self.conv_block(cab_x)
        ...
        # FFN
        x = shortcut + self.wave_up(self.drop_path(attn_x).view(b, int(h/2), int(w/2), int(4*c)).permute(0, 3, 1, 2)+cab_x).permute(0, 2, 3, 1).view(b, h * w, c)  * self.conv_scale
