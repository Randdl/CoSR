...
def main():
    ...
    parser.add_argument('--model_path', type=str,
                        default='/mnt/bn/liuwei/ours/superresolution/ours_sr_classical_x2_hf_3in_merge_hf_ECAB_woCor_RS_ours_dl/models/350000_G.pth')
    parser.add_argument('--folder_lq', type=str, default='/mnt/bn/liuwei/data/RS_Image_test_L2', help='input low-quality test image folder')
    parser.add_argument('--folder_gt', type=str, default='/mnt/bn/liuwei/data/RS_Image_test', help='input ground-truth test image folder')
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        cv2.imwrite(f'{save_dir}/{imgname}_ours.png', output)
...
def test(img_lq, model, args, window_size):
    if args.tile is None:
        ...
        output = model(img_lq,torch.ones(1).cuda(),torch.ones(1).cuda())
    else:
        ...
        for h_idx in h_idx_list:
            for w_idx in w_idx_list:
                ...
                out_patch =  model(in_patch,torch.ones(1).cuda(),torch.ones(1).cuda())
